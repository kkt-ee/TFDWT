...
The four matrices in the two band perfect reconstruction filterbank in \autoref{fig:2BPRFB} are — (i) $\boldsymbol{G}$ is lowpass analysis matrix, (ii) $\boldsymbol{H}$ is highpass analysis matrix, (iii) $\tilde{\boldsymbol{G}}$ is lowpass synthesis matrix and (iv) $\tilde{\boldsymbol{H}}$ is highpass synthesis matrix. These matrices are operators for circular convolution, constructed by circular shifts of the corresponding FIR filters $g\big[n-k\big]$, $h\big[n-k\big]$, $\tilde{g}\big[n-k\big]$ and $\tilde{h}\big[n-k\big]$, where $g=\tilde{g}$ and $h=\tilde{h}$ for orthogonal wavelets.

![Two band perfect reconstruction filterbank.\label{fig:2BPRFB}](figs/DWTFB.jpg){ width=50% }
...
The pixel values in an image are a function of two independent spatial axes. A DWT 2D filterbank is a separable transform with row-wise filtering followed by column-wise filtering that yields four subbands - LL, LH, HL and HH. A two-dimensional discrete wavelet system is,
...
![Separable DWT 2D perfect reconstruction filterbank.\label{fig:2BPRFB-2}](figs/DWTFB2D.jpg){ width=70% }
...
# 5 Multilevel wavelet filterbanks

The above-discussed DWT and IDWT layers are building blocks in constructing multilevel DWT filterbanks. \autoref{fig:MultilevelDWTtiling} shows the partitioning of the 1D frequency axis and tiling of the 2D frequency plane using a level-4 1D and 2D DWT. The multilevel DWT successively decomposes the low-frequency feature. If the high-frequency features are also decomposed successively, then we get a Wavelet Packet Transform (WPT) filterbank.
