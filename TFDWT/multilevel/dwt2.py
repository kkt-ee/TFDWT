...
def idwt2(subbands, level=3, Ψ='haar'):
    """ Multilevel 2D IDWT
    ...
    *highpasses, lowpass = subbands  # unpack: [H1, H2, ..., Hn, ln]
    ...
    for H in reversed(highpasses):
        current = IDWT2D(wave=Ψ)(Concatenate()([current, H]))
    
    return current

if __name__=='__main__':
    batch_size, N, channels = 1, 256, 1
    x = tf.random.normal((batch_size, N, N, channels))
    x.shape
    level = 4
    subbands = dwt2(x, level=level)
    print([_.shape for _ in subbands])
    x_rec = idwt2(subbands, level=level)
    import numpy as np
